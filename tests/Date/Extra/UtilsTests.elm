...
tests =
  let
    ...
    _ = Debug.log "UtilsTests currentOffsets" currentOffsets
    ...
    describe "Date.Utils tests"
      ...
      , describe "Utils.isoWeek - test in matching time zones only." <|
          List.filterMap currentOffsetTest
            [ ( (-180, -120) -- UTC +2:00 Helsinki timezone (in windows its called that)
...
testNeg180HelsinkiIsoWeek _ =
  let
    currentOffsets = TestUtils.getZoneOffsets 2015
    _ = Debug.log "testNeg180HelsinkiIsoWeek currentOffset" currentOffsets
    failMsg =
      """
  ...
  in
    if currentOffsets /= (-180, -1201) then
      test failMsg <|
        \() -> Expect.fail "mooo" -- failMsg
    else
      let
        sep28 = DateUtils.unsafeFromString "2016-3-28"
        sep28Offset = Debug.log "sep28Offset" <| Create.getTimezoneOffset sep28
        sep28isoWeek = DateUtils.isoWeek sep28
        sep27 = DateUtils.unsafeFromString "2016-3-27"
        sep27Offset = Debug.log "sep27Offset" <| Create.getTimezoneOffset sep27
        sep27isoWeek = DateUtils.isoWeek sep27
        sep23 = DateUtils.unsafeFromString "2016-3-23"
        sep23Offset = Debug.log "sep23Offset" <| Create.getTimezoneOffset sep23
        sep23isoWeek = DateUtils.isoWeek sep23
        sep21 = DateUtils.unsafeFromString "2016-3-21"
        sep21Offset = Debug.log "sep21Offset" <| Create.getTimezoneOffset sep21
        sep21isoWeek = DateUtils.isoWeek sep21
      in
      -- Reference: https://www.epochconverter.com/weeks/2016
        describe "Timezone +1000 Sydney (daylight saving)"
          ...
          , test "Check isoWeek sep28 is as expected" <|
            \() -> Expect.equal sep28isoWeek (2016, 13, 1) -- BUG daylight saving change
